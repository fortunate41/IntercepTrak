...
if N > 0
    f = fopen('parfor_progress_percentage.txt', 'w');
    if f<0
...
elseif N == 0
    delete('parfor_progress_percentage.txt');
    percent = 100;
    ...
    if ~exist('parfor_progress_percentage.txt', 'file')
        error('parfor_progress_percentage.txt not found. Run PARFOR_PROGRESS(N) before PARFOR_PROGRESS to initialize parfor_progress.txt.');
    end
    
    f = fopen('parfor_progress_percentage.txt', 'a');
    fprintf(f, '1\n');
    fclose(f);
    
    f = fopen('parfor_progress_percentage.txt', 'r');
    progress = fscanf(f, '%d');
    fclose(f);
