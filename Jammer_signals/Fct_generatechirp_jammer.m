function [chirp_wave]=Fct_generatechirp_jammer( fs, SweepRange_Hz, SweepPeriod_s, upchirp_true,Time_window_s, phi0,f_IF_Hz);
...
if upchirp_true==1
    chirp_freq=f_IF_Hz+mu_chirp/2*time_ax1 +phi0./(2*pi*time_ax1);  %go from fmin to fmax in Tsym
else
    chirp_freq=f_IF_Hz- mu_chirp/2*time_ax1 +phi0./(2*pi*time_ax1);%go from fmax to fmin in Tsym
end

chirp_wave=exp(1j*2*pi*chirp_freq.*time_ax1);
